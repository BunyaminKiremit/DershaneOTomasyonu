...
namespace DersHaneOTM
{
    public partial class OgrenciGüncelle : Form
    {
        ...
        {
            con.Open();
            SqlCommand cmd=new SqlCommand("update OgrenciKayıt set ad=@ad,soyad=@soyad,tckimlik=@tckimlik,dogumtarihi=@dogumtarihi,cinsiyet=@cinsiyet,telefon=@telefon,alan=@alan,adres=@adres,kayıttarihi=@kayıttarihi,velitc=@velitc,veliad=@veliad,velisoyad=@velisoyad,velitelefon=@velitelefon where numara='"+dataGridView1.CurrentRow.Cells[0].Value.ToString()+"'",con);
        ...
        private void OgrenciGüncelle_Load(object sender, EventArgs e)
        {
